...
from app.utils_geocode import map_addresses, geocode_addresses
...
with col12_left:
    ...
    def build_full_address(gu: pd.Series, addr: pd.Series) -> pd.Series:
        ...
        for g, a in zip(gu, addr):
            s = a
            # 이미 '서울'이나 '경기', '인천'으로 시작하면 그대로 두고,
            # 아니면 '서울특별시' 접두를 붙임
            if s and not s.startswith(("서울", "경기", "인천")):
                # 주소가 구로 시작하지 않으면 구도 보강
    ...
    base_df["addr_full"] = build_full_address(base_df["gu"], base_df["address"])

    # [A] 1차 진단: 키/주소 상태 확인 (지오코딩 호출 전)
    st.write({
        "ENV_KEY_EXISTS": bool(KAKAO_REST_API_KEY),
        "ENV_KEY_LEN": len(KAKAO_REST_API_KEY or ""),
        "rows_total": len(base_df),
        "address_nonempty": int(base_df["address"].fillna("").str.strip().ne("").sum()),
    })
    st.dataframe(base_df[["name", "gu", "address"]].head(10))

    # 1) 원천 데이터 검사
    if base_df.empty:
        st.warning("⚠️ 해당 구에 원천 CSV 데이터가 없습니다. (CSV 내용 확인)")
        st.stop()

    # 2) 주소→좌표 매핑 (캐시 우선)
    df_geo = map_addresses(base_df, addr_col="addr_full")
    rows_total = len(df_geo)
    have_coords = int(df_geo["lat"].notna().sum())
    missing = rows_total - have_coords

    # 2-1) 캐시가 전혀 없으면 1회 자동 지오코딩 시도
    if have_coords == 0 and rows_total > 0:
        st.info("ℹ️ 좌표 캐시가 비어 있어 자동 지오코딩을 1회 시도합니다...")
        geocode_addresses(base_df["addr_full"])
        # 다시 매핑
        df_geo = map_addresses(base_df, addr_col="addr_full")
        # [B] 지오코딩 직후 진단: 캐시/좌표 생성 결과 확인
        st.write({
            "ENV_KEY_EXISTS": bool(KAKAO_REST_API_KEY),
            "ENV_KEY_LEN": len(KAKAO_REST_API_KEY or ""),
            "rows_total": len(base_df),
            "address_nonempty": int(base_df["addr_full"].fillna("").str.strip().ne("").sum()),
        })
        st.dataframe(base_df[["name", "gu", "address", "addr_full"]].head(10))

        have_coords = int(df_geo["lat"].notna().sum())
        missing = len(df_geo) - have_coords

    # 2-2) 아직 누락이 있으면 보충 버튼 제공
    if missing > 0:
        if st.button(f"지오코딩 보충 실행 ({missing}건 누락)"):
            geocode_addresses(base_df["address"])
            st.rerun()

    # 3) 지도에 표시할 데이터(좌표 있는 것만)
    df_map = df_geo.dropna(subset=["lat", "lon"]).reset_index(drop=True)

    # 3-1) 좌표가 여전히 전혀 없으면 원인 안내
    if df_map.empty:
        st.error(
            "❌ 지도에 표시할 좌표가 없습니다.\n\n"
            "- `.env`에 **KAKAO_REST_API_KEY**가 정확한지 확인\n"
            "- **address** 컬럼에 실제 도로명/지번 주소가 들어있는지 확인\n"
            "- (권장) `python -m scripts.batch_geocode`로 캐시 선반영"
        )
        # 필요시 임시로 구 중심좌표라도 보여주고 싶다면 아래 2줄 주석 해제
        # df_map = attach_latlon_by_gu_centroid(base_df).assign(_fallback=True)
        # st.info("임시로 구 중심좌표를 사용해 표시합니다. (지오코딩 성공 시 자동 대체)")
        # 그래도 중단
        st.stop()
