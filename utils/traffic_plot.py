...
def load_speed_long_csv(csv_path: Path) -> pd.DataFrame:
    """CSV: its_link_id, 시간대, 평균속도(km/h), hour"""
    df = pd.read_csv(csv_path)
    df["its_link_id"] = df["its_link_id"].astype(str)
...
def get_nearby_speed_data(
    ...
    """
    CSV(its_link_id, 시간대, 평균속도(km/h), hour) + 레벨6 SHP로
    ...
    center = gpd.GeoSeries([Point(center_lon, center_lat)], crs=4326).to_crs(
        epsg=3857
    ).iloc[0]
    ...
    up_ids = near["up_its_id"].round().astype(int).astype(str)
    dw_ids = near["dw_its_id"].round().astype(int).astype(str)
    its_ids = {i for i in up_ids if i != "0"} | {i for i in dw_ids if i != "0"}

    speed = load_speed_long_csv(csv_path)
    df_plot = speed[speed["its_link_id"].isin(its_ids)].copy()

    if df_plot.empty:
        return df_plot

    keep = df_plot["its_link_id"].value_counts().head(max_links).index
    df_plot = df_plot[df_plot["its_link_id"].isin(keep)].copy()
    df_plot = df_plot.sort_values(["its_link_id", "hour"])
    return df_plot
...
def altair_nearby_speed(df_plot: pd.DataFrame, height: int = 800):
    ...
    selection = alt.selection_point(fields=["its_link_id"], bind="legend")
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color("its_link_id:N", title="ITS 링크"),
            opacity=alt.condition(selection, alt.value(1.0), alt.value(0.25)),
            tooltip=[
                alt.Tooltip("its_link_id:N", title="링크"),
...
def plot_speed(
    ...
    df_plot = get_nearby_speed_data(
        csv_path, shp_path, center_lon, center_lat, radius_m, max_links
    )
    ...
    if renderer == "altair":
        ...
        if df_plot.empty:
            ax.set_title("반경 내 ITS 링크에 대응하는 속도 데이터가 없습니다.")
            return fig, df_plot
        for lid, sub in df_plot.groupby("its_link_id"):
        ...
        if df_plot.empty:
            ...
            fig = go.Figure().update_layout(
                title="반경 내 ITS 링크에 대응하는 속도 데이터가 없습니다."
            )
            return fig, df_plot
        fig = px.line(
            df_plot,
